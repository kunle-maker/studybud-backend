...
import { askSubjectTeacher } from '../services/aiService.js';
...
export const askSubject = asyncHandler(async (req, res) => {
  ...
  if (!subject?.trim() || !branch?.trim()) {
    return res.status(400).json({ success: false, message: 'Subject and branch are required.' });
  }
  ...
  const answer = await askSubjectTeacher(question, context, subject, branch, req.user.role);
